...
# agarPad modules
from APLibrary import plot_contours, contour_stats1, contour_stats2, crop_cell_box
...
def pc_profile_sizing(img, contours, params):
    ...
    for h, cnt in reversed(list(enumerate(contours))):
        ...
        # crop out a rectangle around the cell and rotate it
        b_pc = crop_cell_box(img, cnt, 15, 5, params)
        ...
        d_pc = b_pc[sz_imgR/2-3:sz_imgR/2+3,:]
        ...
        len_kkk = len(kkk_pc)
        if (len_kkk)==0:
            if params['debug_phase']: print('throw out bad length profile')
            del contours[h]
            continue
        ...
        try:
            ...
            if params['debug_phase']: print('normalization failed length')
            del contours[h]
            continue
        ...
        if np.isnan(r_th) or np.isnan(l_th) or np.isnan(r_th-l_th) or r_th==0 or l_th==0:
            if params['debug_phase']: print('throwout bad pre length')
            del contours[h]
            continue
        else:
            ll = r_th - l_th # length
        ...
        # split width into two parts to avoid constricted centers
        try:
            ll_b=int(l_th+wd/2)
        ...
        for pxl in range(cc):
            ...
            try:
                ...
                if params['debug_phase']: print('normalization failed width')
                del contours[h]
                continue
        ...
        if np.isnan(l_th_mean) or np.isnan(r_th_mean) or np.isnan(r_th_mean-l_th_mean) or l_th_mean==0 or r_th_mean==0:
            if params['debug_phase']: print('throwout bad mean')
            del contours[h]
            continue
        else:
            ww=r_th_mean-l_th_mean
            ...
            if np.isnan(ll) or np.isnan(ww):
                if params['debug_phase']: print('throwout ll or ww is NaN')
                del contours[h]
                continue
            elif ll <= params['lth_cut_low']/pxl2um or \
              ww <= params['wth_cut_low']/pxl2um:
                if params['debug_phase']: print('throwout pc cell small')
                del contours[h]
                continue
            elif ll > params['lth_cut_up']/pxl2um or \
              ww > params['wth_cut_up']/pxl2um:
                if params['debug_phase']: print('throwout pc cell big')
                del contours[h]
                continue
            else:
        ...
        if params['debug_phase']:
            ...
            img_title = params['path_debug'] + 'pc_thr_cell_' + str(h) + '_' + ('%2.2f' % ll) + '_' + ('%2.2f' % ww) + '.jpg'
