...
def find_contours(img, params):
    ...
    contours
        list of contours
        Each contour is a list of points in xy
    '''
    ...
    img_inv = cv2.equalizeHist(closing)
    
    # increase contrast

    enhanced_img = cv2.equalizeHist(img_inv)
    ...
    # way, saving memory.
    contours, hierarchy = cv2.findContours(enhanced_img, cv2.RETR_EXTERNAL,
...
def filter_contours(img, contours, params, pc_name = ""):
    ...
    if params['debug']:
        ...
        util_local.create_folder(params["path_debug"])
        if len(pc_name)>2:
            pc_name=pc_name[:-2]
        else:
            pass
        ...
        skimage.io.imsave(params["path_debug"]+ pc_name[:-4] +"contours_before.png", con_img_before)
        skimage.io.imsave(params["path_debug"]+ pc_name[:-4] +"contours_after.png", con_img_after)
...
def contour_stats1(contour):
    ...
    box = cv2.boxPoints(rect) # turn that rect object into 4 points
    box = np.int0(box)
...
#        plt.imshow(img, interpolation='nearest', cmap='gray')
    ...
    img_cropped = rotation_crop_padded[int(sz_imgR/2-y_pad) : int(sz_imgR/2+y_pad), 
                                       int(sz_imgC/2-x_pad) : int(sz_imgC/2+x_pad)]
