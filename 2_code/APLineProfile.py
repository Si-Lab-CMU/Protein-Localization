...
################################################################################
### functions
def line_profiling(img, imgfluo, contours, line_profile, params):
    '''line_profiling generates the line profile of fluorescence internsity acroos the cell
    long and short axis
    ...
    # find background intensities of the whole image and do background substraction
    ...
    imgfluo = imgfluo - mean_background[0]
    ...
    for h, cnt in reversed(list(enumerate(contours))):
        ...
        # crop out a rectangle around the cell - fluorescence image
        s_piece_fl = imgfluo[np.int0(rect[0][1]-sz_img/2):np.int0(rect[0][1]+sz_img/2), np.int0(rect[0][0]-sz_img/2):np.int0(rect[0][0]+sz_img/2)]
        # rotate image so cell is horizontal - fluorescence image
        b_fl = ndimage.rotate(s_piece_fl, angle)
...
#        img_piece=f_pc[0:r, ll_b:rr_b]
        ...
        ####################################################################
        ### generate the line profile of fluorescence image (along short-axis)

        sz_imgF = b_fl.shape[0]
        f_fl = b_fl[sz_imgF//2-sz_wth//2-3:sz_imgF//2+sz_wth//2+5, sz_imgF//2-sz_lth//2:sz_imgF//2+sz_lth//2]
        ...
        l_img=f_fl[0:r, ll_b:lr_b]
        r_img=f_fl[0:r, rl_b:rr_b]

        # Define the range to average the width
#        img_piece=f_fl[0:r, ll_b:rr_b]
        img_piece=np.concatenate((l_img, r_img),axis=1) #combine two parts that do not include septum FS20210714
        rr, cc = img_piece.shape
        ...
        for pxl in range(cc):
            try:
                kkk_fl_w=img_piece[0:r, pxl]
            except:
                del contours[h]
                continue
            #kkk_fl_w=np.mean(s_fl,1)
            len_kkk=len(kkk_fl_w)
            if (len_kkk)==0:
                del contours[h]
                continue
            ...
            mx_fl = max(kkk_fl_w)
            mn_fl = min(kkk_fl_w)
            mean_fl = np.mean(kkk_fl_w)

            # normalize data
            try:
#                kkk_fl_r = 1 - (kkk_fl_w-mn_fl) / (mx_fl-mn_fl)
                kkk_fl_r = kkk_fl_r + kkk_fl_w/mean_fl
#                kkk_fl_r = kkk_fl_w/mean_fl
            except:
                del contours[h]
                continue
        kkk_fl_r = kkk_fl_r/cc
        # print([xxx_w, kkk_fl_r, l_th_mean, r_th_mean])
        line_profile.append([xxx_w, kkk_fl_r, l_th_mean, r_th_mean])            
        ...
        if params['debug_phase']:
            ...
            plt.xlabel('pixels')
            plt.ylabel('pixels')
            plt.imshow(b_fl, cmap=plt.cm.gray, interpolation='nearest')
    ...
    return contours, lth_pc, wth_pc, line_profile
    print(line_profile)
